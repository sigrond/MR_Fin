...
inline void GeneratePatternPT(int sizeTheta, real *  II, real *  r, int sizeR, real complex const * const m, real *  pii, real *  tau,
...
#pragma omp for nowait
	for(int j=0;j<sizeR;j++)
		...
		//nMax[j] = (int)ceil(correctedR[((j+3)/4)*4]+4.0*pow(correctedR[((j+3)/4)*4], (real)1/3)+2.0);
	}
...
#pragma omp parallel for
	...
	//for(int i=0;i<sizeR;i++)
		//printf("%d\n", nMax[i]);
#ifdef CUDA
	if(polarization==0) cudaGenerate(sizeR, pattern_length, nMax, pii, nPiiTau, tau, aReal, aImag, bReal, bImag, II, polarization );
	else                cudaGenerate(sizeR, pattern_length, nMax, tau, nPiiTau, pii, aReal, aImag, bReal, bImag, II, polarization );
#endif
